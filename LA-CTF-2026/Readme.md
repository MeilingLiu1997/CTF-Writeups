...
Solved: 9 challenges  
Categories: web, crypto, misc, rev


# rev/ooo
**Approach:** Static analysis of [ooo.py](ooo.py)
Identified simple transformation logic and reversed it to recover the flag.
...
**Type:** Client-side bypass  

The page logic relied on `setInterval` to restrict behavior.  
Clearing all intervals in the browser console allowed direct manipulation.

```js
for (let i = 0; i < 1000; i++) clearInterval(i);
disp.textContent = "flag";
```
...
The challenge used UTF-16LE encoding interpreted as UTF-8.
...
**Type:** Frontend inspection
Paused execution in DevTools → inspected source → extracted hardcoded flag.
...
**Type:** RSA prob, n is not a prime

Modulus `n` was not a product of two large secure primes.
Factored `n`, computed private key, decrypted ciphertext.
...
[ct.txt](ct.txt)
**Exploit:** [bigramcrash.py](bigramcrash.py)
...
**Type:** Server-Side HTML Injection
...
**Exploit:**[clockcrash.py](clockcrash.py)
